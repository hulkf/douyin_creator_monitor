...
后续所有带有文件输入框或命令行输入参数的功能，统一接受单个文件或目录。目录只处理当前目录下的文件，不递归处理下级目录；路径中包含空格或用户输入时包裹的单引号、双引号都必须正常工作。
...
规范化作品文件继续保存完整历史，但流水线下游只选择 `pending_aweme_ids` 中尚未完成主链路的新增作品，以及 `delivery_retry_aweme_ids` 中待重试的可选交付。`--max-works` 只限制本轮处理数量，未处理完的 ID 会继续留在对应队列。IMA、夸克或 Obsidian 单独失败时，已完成的采集、ASR 和飞书写入会从主 pending 队列移除，只重试失败的交付目标。
...
默认会跳过已经成功的逐作品阶段，并复用已有的原始转写和纠正后文案。飞书同步会分页读取全部作品，但只更新内容发生变化的记录；北京时间字符串和飞书返回的 ISO 8601 日期会按同一时间点比较，避免只因显示格式不同而误判回读失败。新作品批量创建同时受“每批最多 200 条”和“单批 JSON 最多 20,000 字符”限制，并把返回的 `record_id` 按作品顺序直接传给文案和状态回写。需要强制重跑某一步时使用：
...
每个外部命令默认受 `timeouts.default_seconds` 限制（默认 3600 秒），超时会终止整棵子进程并记录为 `timeout`。毫秒级耗时、状态、失败调用数以及整轮墙钟时间都会写入运行摘要的 `timings` 和 `wall_seconds`；运行摘要还分别提供 `core_status`、`profile_status`、`delivery_status` 和分类计数，便于区分主链路失败与可选交付降级。
...
- “项目总览”只展示达人规模、作品与待处理数量、历史成功率、账号池等累计统计，不混入当次执行明细；
- “当前任务”独立展示本次计划、达人执行明细、事件时间线和可折叠技术日志；
- “历史运行”从 `runtime/pipeline/runs/*.json` 汇总上一次、上上次及更多运行结果，以普通用户可读的成功/异常摘要展示，不直接暴露 Traceback 或本机路径；
- “立即运行”只调用 `schtasks /run /tn DouyinCreatorMonitor`，不会从 Web 服务直接运行 Python 流水线；
- 配置页按基础设置、账号池、作品采集、飞书、ASR、内容总结、备份策略和达人信息分别展示栏目，一次只显示当前栏目；IMA、夸克和 Obsidian 均集中在“备份策略”页内分组管理；
- “作品采集”可选择无头浏览器或可见浏览器。无头模式保持原有定时任务行为；关闭“使用无头浏览器”后会打开可见 Chrome，便于观察采集过程，也可能降低账号被限制的概率；
- 达人信息使用一套统一表单模板，通过达人列表切换当前编辑对象，不会为每位达人生成独立配置页面；
- 账号池可预先维护多个账号槽位并逐个扫码登录；每个槽位使用独立、持久化的 MediaCrawler Chromium Profile，登录操作只建立登录态、不采集作品；
- 同一 Profile 带跨进程互斥保护：每日流水线运行时不能发起扫码，扫码未结束时 Web 页面也不会启动每日任务；
...
即可打开“抖音达人监控中心”。GUI 使用 Python 标准库 Tkinter，不需要额外安装前端或 GUI 依赖。

当前页面提供：

- Windows 定时任务状态、下次运行时间和最近退出码；
- 最近一轮流水线状态、耗时和日志；
- 每位达人作品总数、待处理数、数据更新时间和异常原因；
- 抖音账号池配置数量及检测到本地登录文件的 profile 数量（不代表在线有效或未风控）；
- “立即运行”按钮：只调用 `schtasks /run /tn DouyinCreatorMonitor`，不会从 GUI 直接运行 Python 流水线；
- 刷新状态、打开本地配置和打开最新日志。

页面默认每 12 秒自动刷新。GUI 只是本地控制和观察层，关闭窗口不会停止已经由 Windows 任务计划程序启动的采集任务。
...
- 每轮首次 IMA 写入前先调用官方只读接口检查凭证；401、`code 200002` 或 `skill auth failed` 会在整轮范围熔断 IMA，其他目标和达人继续。超时、429、5xx、DNS 和临时连接错误不会熔断。
...
- 作品已采集但达人资料缺少 IP 属地等字段时，作品继续进入主链路，达人资料写回标记为降级，不再为同一批作品串行重抓。
...
- 计划任务启用错过启动后补跑，最长单次执行 6 小时；`run_scheduled.bat` 启动前滚动超过 5 MiB 的入口日志，并清理 60 天前的已知运行日志。
